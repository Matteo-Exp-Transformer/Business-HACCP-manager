...
import TemperatureModeInput from './ui/TemperatureModeInput'
...
import { HACCP_TOLERANCE_C, AMBIENT_RANGE_C, LOCALSTORAGE_KEYS } from '../utils/haccpConstants'
...
import { TEMP_MODES, createConservationPoint } from '../utils/dataModels'
import { migrateConservationPoints, isConservationPointNormalized, autoMigrateData } from '../utils/dataMigration'
import { debugLog, haccpLog, tempLog, maintenanceLog, migrationLog } from '../utils/debug'
import { checkHACCPCompliance, checkTemperatureCategoryConflicts, validateConservationPoint, validateMaintenanceConfig } from '../utils/haccpValidation'

// Usa le categorie HACCP standardizzate
const STORAGE_CATEGORIES = CONSERVATION_POINT_RULES.categories

function ConservationPoints({ temperatures, setTemperatures, currentUser, refrigerators, setRefrigerators, departments, setDepartments }) {
  ...
  useEffect(() => {
    ...
    console.log('🔍 formData aggiornato:', formData);
    setForceUpdate(prev => prev + 1)
  }, [formData.setTempMode, formData.setTempC, formData.setTempRangeC])
    ...
    setFormData(prev => {
      ...
      if (isSelected) {
        ...
        if (currentCategories.length < 5) {
          ...
          alertWarning('Puoi selezionare al massimo 5 categorie')
          return prev
        }
      }
    })
  ...
  const checkTemperatureCategoryConflicts = (temperature, categories) => {
    ...
    categories.forEach(categoryId => {
      ...
      if (category) {
        const optimalTemp = getOptimalTemperatureLocal(categoryId)
        if (optimalTemp) {
          // Gestisce sia il formato legacy (stringa) che il nuovo formato (oggetto)
          let tempValue
          if (typeof temperature === 'string') {
            tempValue = temperature.toLowerCase().trim() === 'ambiente' ? 20 : parseFloat(temperature)
          } else if (typeof temperature === 'object' && temperature.setTempMode) {
            // Nuovo formato normalizzato
            if (temperature.setTempMode === TEMP_MODES.FIXED) {
              tempValue = temperature.setTempC
            } else if (temperature.setTempMode === TEMP_MODES.RANGE) {
              tempValue = (temperature.setTempRangeC.min + temperature.setTempRangeC.max) / 2
            } else {
              tempValue = 20 // Ambiente
            }
          } else {
  ...
  useEffect(() => {
    ...
    if (savedRefrigerators) {
      // Controlla se i dati sono già normalizzati
      const needsMigration = savedRefrigerators.some(ref => !isConservationPointNormalized(ref))
      
      let updatedRefrigerators
      if (needsMigration) {
        migrationLog('Migrando punti di conservazione al formato normalizzato...')
        updatedRefrigerators = migrateConservationPoints(savedRefrigerators)
        setLocalStorageData(LOCALSTORAGE_KEYS.REFRIGERATORS, updatedRefrigerators)
      } else {
        updatedRefrigerators = savedRefrigerators
      }
      
      setRefrigerators(updatedRefrigerators)
      
      // Debug: mostra la classificazione di ogni punto
      if (needsMigration) {
        haccpLog('Classificazione punti di conservazione:')
        updatedRefrigerators.forEach(ref => {
          const type = getRefrigeratorType(ref)
          const displayTemp = getDisplayTemperature(ref)
          haccpLog(`   ${ref.name}: ${displayTemp}, isAbbattitore: ${ref.isBlastChiller} → ${type}`)
        })
      }
    }
  }, [])
    ...
    const loadMaintenanceData = async () => {
      try {
        ...
        if (result.success && result.data.length > 0) {
          debugLog('🔍 Caricamento attività di manutenzione dal database:', result.data.length)
          ...
          maintenanceLog('Attività raggruppate:', maintenanceArray)
          maintenanceLog('Numero totale di gruppi di attività:', maintenanceArray.length)
          ...
          setRefrigerators(prevRefrigerators => {
            ...
            if (hasMaintenanceData) {
              maintenanceLog('Dati di manutenzione già presenti, saltando aggiornamento')
            ...
            const updatedRefrigerators = prevRefrigerators.map(refrigerator => {
              ...
              if (pointMaintenances && pointMaintenances.tasks) {
                ...
                debugLog(`🔍 Aggiornando ${refrigerator.name} con maintenanceData:`, maintenanceData)
                debugLog(`🔍 assigned_staff_ids:`, maintenanceData.temperature_monitoring?.assigned_staff_ids, maintenanceData.sanitization?.assigned_staff_ids, maintenanceData.defrosting?.assigned_staff_ids)
                return {
                  ...refrigerator,
        ...
        errorLog('❌ Errore nel caricamento delle attività di manutenzione:', error)
      }
    }
  ...
  useEffect(() => {
    ...
    if (savedStaff) {
      try {
        ...
        debugLog('🔍 Caricando staffMembers dal localStorage:', staff.length)
        debugLog('🔍 StaffMembers:', JSON.stringify(staff, null, 2))
        setStaffMembers(staff)
      } catch (error) {
        errorLog('Errore nel caricamento dello staff:', error)
      }
    } else {
      debugLog('🔍 Nessun staffMembers trovato nel localStorage')
    }
  }, [])
  ...
  useEffect(() => {
    ...
    if (savedOnboarding && savedOnboarding !== '[object Object]' && !existingRefrigerators && !hasLoadedOnboarding) {
      try {
        ...
        debugLog('🔍 Caricando staffMembers dall\'onboarding:', data.formData.staff.staffMembers.length)
        debugLog('🔍 Primo dipendente:', JSON.stringify(data.formData.staff.staffMembers[0], null, 2))
        setStaffMembers(data.formData.staff.staffMembers)
        } else {
          debugLog('🔍 Nessun staffMembers trovato nell\'onboarding')
        }
        ...
        if (data.formData?.conservation?.points && Array.isArray(data.formData.conservation.points)) {
          ...
          debugLog('🔍 Dati onboarding caricati:', data.formData.conservation.points.length, 'punti,', savedMaintenances.length, 'attività manutenzione')
          debugLog('🔍 Primo punto di conservazione:', data.formData.conservation.points[0])
          ...
          const conservationPoints = data.formData.conservation.points.map(point => {
            ...
            debugLog(`🔍 Punto ${point.name} - MaintenanceData finale:`, maintenanceData)
            debugLog(`🔍 Punto ${point.name} - assigned_staff_ids:`, maintenanceData.temperature_monitoring?.assigned_staff_ids, maintenanceData.sanitization?.assigned_staff_ids, maintenanceData.defrosting?.assigned_staff_ids)
            
            debugLog(`🔍 Punto ${point.name} - targetTemp:`, point.targetTemp, 'setTemperature:', point.setTemperature, 'temperature:', point.temperature)
            debugLog(`🔍 Punto ${point.name} - Tutti i campi temperatura:`, {
              targetTemp: point.targetTemp,
              setTemperature: point.setTemperature,
              temperature: point.temperature,
              temp: point.temp
            })
            debugLog(`🔍 Punto ${point.name} - Punto completo:`, JSON.stringify(point, null, 2))
          ...
          setLocalStorageData(LOCALSTORAGE_KEYS.REFRIGERATORS, conservationPoints)
          
          // Marca l'onboarding come caricato per evitare ricaricamenti
          localStorage.setItem('haccp-onboarding-loaded', 'true')
          
          haccpLog('✅ Punti di conservazione caricati dall\'onboarding:', conservationPoints.length, 'punti')
        }
      } catch (error) {
        errorLog('Errore nel caricamento dei dati onboarding:', error)
      }
    }
  }, [])
  ...
  const addCustomCategory = () => {
    if (!newCategoryData.name.trim() || !newCategoryData.description.trim()) {
      alertError('Nome e descrizione sono obbligatori')
      return
    }
    ...
    // Mostra conferma
    alertSuccess(`Categoria "${newCategory.name}" aggiunta con successo!`)
  }

  // Funzione per eliminare una categoria personalizzata
  const deleteCustomCategory = (categoryId) => {
    confirmDelete(
      'Sei sicuro di voler eliminare questa categoria? I frigoriferi che la utilizzano non avranno più una categoria assegnata.',
      () => {
  ...
  // Rimuovo la funzione duplicata - ora usa quella importata da temperatureHelpers
  ...
  // Helper function to proceed with refrigerator creation after conflict check
  const proceedWithRefrigeratorCreation = async () => {
    ...
    // Crea il punto di conservazione normalizzato
    const newRefrigerator = createConservationPoint({
      name: formData.name.trim(),
      setTempMode: formData.setTempMode,
      setTempC: formData.setTempC,
      setTempRangeC: formData.setTempRangeC,
      departmentName: formData.location.trim(),
      storageCategoryIds: finalCategories,
      isBlastChiller: formData.isAbbattitore,
      maintenance: formData.maintenanceData || {},
      createdAt: new Date().toISOString(),
    ...
    if (formData.maintenanceData && Object.keys(formData.maintenanceData).length > 0) {
      try {
        ...
        if (result.success) {
          maintenanceLog('✅ Attività di manutenzione salvate:', result.data);
        } else {
          errorLog('❌ Errore nel salvataggio manutenzione:', result.error);
        }
      } catch (error) {
        errorLog('❌ Errore durante il salvataggio delle attività di manutenzione:', error);
      }
    }
  ...
  const addRefrigerator = async (e) => {
    ...
    if (!formData.name.trim() || !formData.location.trim() || !formData.selectedCategories || formData.selectedCategories.length === 0) {
      return
    }
    
    // Validazione temperatura basata sul modo
    if (formData.setTempMode === TEMP_MODES.FIXED && (!formData.setTempC || isNaN(formData.setTempC))) {
      alertError('Inserisci una temperatura valida per il modo fisso')
      return
    }
    
    if (formData.setTempMode === TEMP_MODES.RANGE && (!formData.setTempRangeC || !formData.setTempRangeC.min || !formData.setTempRangeC.max)) {
      alertError('Inserisci un range di temperatura valido')
      return
    }
    ...
    if (formData.maintenanceData) {
      ...
      if (hasMaintenanceErrors) {
        alertError('Configurazione manutenzione incompleta. Verifica che tutte e tre le attività (Rilevamento Temperatura, Sanificazione, Sbrinamento) abbiano frequenza, ruolo e categoria assegnati.')
        return
      }
    }
    ...
    if (existingRefrigerator) {
      alertError(`Un punto di conservazione con questo nome esiste già: "${existingRefrigerator.name}" (creato il ${new Date(existingRefrigerator.createdAt).toLocaleDateString('it-IT')}). Scegli un nome diverso.`)
      return
    }
    ...
    if (formData.selectedCategories && formData.selectedCategories.length > 0) {
      ...
      const conflicts = checkTemperatureCategoryConflicts(tempPoint, formData.selectedCategories)
      
      if (conflicts.length > 0) {
        const temperatureDisplay = getDisplayTemperature(tempPoint)
        ...
        confirmAction(
          ...
          () => {
            ...
            proceedWithRefrigeratorCreation()
          },
          () => {
  ...
  const deleteRefrigerator = async (id) => {
    confirmDelete(
      'Sei sicuro di voler eliminare questo punto di conservazione?',
      async () => {
      try {
        ...
        if (result.success) {
          maintenanceLog('Attività di manutenzione eliminate per il punto:', id);
        } else {
          errorLog('❌ Errore nell\'eliminazione manutenzione:', result.error);
        }
      } catch (error) {
        errorLog('❌ Errore durante l\'eliminazione delle attività di manutenzione:', error);
      }
  ...
  const editRefrigerator = (refrigerator) => {
    ...
    haccpLog('EditRefrigerator - Dati del punto:', refrigerator.name)
    maintenanceLog('maintenanceData completo:', JSON.stringify(refrigerator.maintenanceData, null, 2))
    debugLog('staffMembers disponibili:', staffMembers.length, 'dipendenti')
    
    // Gestisce sia il formato normalizzato che quello legacy
    let tempData = {
      setTempMode: TEMP_MODES.AMBIENT,
      setTempC: undefined,
      setTempRangeC: undefined
    ...
    if (refrigerator.setTempMode) {
      // Formato normalizzato
      tempData = {
        setTempMode: refrigerator.setTempMode,
        setTempC: refrigerator.setTempC,
        setTempRangeC: refrigerator.setTempRangeC
      }
    } else {
      // Formato legacy - usa parseSetTemperature per convertire
      const tempInfo = parseSetTemperature(refrigerator)
      tempData = {
    ...
    tempLog('Dati temperatura normalizzati:', tempData)
    
    const formDataToSet = {
      name: refrigerator.name,
      ...tempData,
      location: refrigerator.location || refrigerator.departmentName || '',
      ...
      selectedCategories: refrigerator.selectedCategories || refrigerator.storageCategoryIds || [],
      maintenanceData: refrigerator.maintenanceData || refrigerator.maintenance || {},
      isAbbattitore: refrigerator.isAbbattitore || refrigerator.isBlastChiller || false
    }
    
    maintenanceLog('FormData da impostare:', JSON.stringify(formDataToSet.maintenanceData, null, 2))
    setFormData(formDataToSet)
    setShowEditModal(true)
  }

  // Helper function to proceed with refrigerator update after conflict check
  const proceedWithRefrigeratorUpdate = async () => {
    // Se è un abbattitore, aggiungi le categorie specifiche dell'abbattitore
    let finalCategories = formData.selectedCategories || []
    if (formData.isAbbattitore) {
      // Aggiungi le categorie specifiche dell'abbattitore
    ...
    // Crea il punto di conservazione aggiornato normalizzato
    const updatedRefrigerator = {
      ...
      departmentName: formData.location.trim(),
      storageCategoryIds: finalCategories,
      isBlastChiller: formData.isAbbattitore || false,
      maintenance: formData.maintenanceData || {},
      updatedAt: new Date().toISOString(),
      updatedBy: currentUser?.name || 'Unknown',
      // Mantieni compatibilità con campi legacy per ora
    ...
    if (formData.maintenanceData && Object.keys(formData.maintenanceData).length > 0) {
      try {
        ...
        const result = await supabaseService.saveMaintenanceTasks(maintenanceTasks);
        if (result.success) {
          maintenanceLog('✅ Attività di manutenzione aggiornate:', result.data);
        } else {
          errorLog('❌ Errore nell\'aggiornamento manutenzione:', result.error);
        }
      } catch (error) {
        errorLog('❌ Errore durante l\'aggiornamento delle attività di manutenzione:', error);
      }
    }
  ...
  const updateRefrigerator = async (e) => {
    ...
    if (existingRefrigerator) {
      alertError('Un punto di conservazione con questo nome esiste già. Scegli un nome diverso.')
      return
    }
  ...
  // Rimuovo le funzioni duplicate - ora usa quelle importate da temperatureHelpers

  // Helper per ottenere lo status della temperatura con la temperatura effettiva
  const getTemperatureStatusWithActual = (refrigerator) => {
    ...
    const status = getTemperatureStatus(lastTemperature.temperature, refrigerator)
                ...
                <div className="border rounded-lg p-4 bg-blue-50">
                  ...
                  <div className="space-y-2">
                    {filteredRefrigerators
                      ...
                      .map(refrigerator => {
                        const status = getTemperatureStatusWithActual(refrigerator)
                        return (
          ...
          <div className="bg-white rounded-lg p-6 sm:p-8 w-full max-w-6xl mx-2 sm:mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={addRefrigerator} className="space-y-4">
              ...
              <div className="bg-blue-50 p-3 rounded-lg">
                ...
                {getTemperatureFromForm(formData) !== null && formData.selectedCategories.length > 0 && (
              ...
              <div className="bg-green-50 p-3 rounded-lg">
                ...
                <div>
                  <TemperatureModeInput
                    value={{
                      setTempMode: formData.setTempMode,
                      setTempC: formData.setTempC,
                      setTempRangeC: formData.setTempRangeC
                    }}
                    onChange={(tempData) => setFormData({...formData, ...tempData})}
                    showValidation={true}
                    selectedCategories={formData.selectedCategories}
                    className="mt-1"
                  />
                  <p className="text-xs text-gray-600 mt-2 p-2 bg-white rounded border">
                    Seleziona la modalità di temperatura e inserisci i valori appropriati
                  </p>
                  
                  {/* Checkbox Abbattitore - appare solo se temperatura è tra -1°C e -90°C */}
                  {(() => {
                    let tempValue
                    if (formData.setTempMode === TEMP_MODES.FIXED) {
                      tempValue = formData.setTempC
                    } else if (formData.setTempMode === TEMP_MODES.RANGE) {
                      tempValue = formData.setTempRangeC ? (formData.setTempRangeC.min + formData.setTempRangeC.max) / 2 : null
                    } else {
                      tempValue = null
                    }
                    
                    const isInAbbattitoreRange = tempValue !== null && !isNaN(tempValue) && tempValue >= -90 && tempValue <= -1;
                ...
                {formData.selectedCategories.length > 0 && (formData.setTempC !== undefined || formData.setTempRangeC !== undefined || formData.setTempMode === TEMP_MODES.AMBIENT) && (
                  <div className="mt-4 p-4 rounded-lg bg-gray-50 border">
                    ...
                    {(() => {
                      ...
                      const compliance = checkHACCPCompliance(tempPoint, formData.selectedCategories);
                      return (
                        <div className={`flex items-center gap-2 ${
...
export default ConservationPoints 
